...
const MyProperties = () => {
  ...
  // Initialize hoveredImages state when properties change
  useEffect(() => {
    if (properties) {
      setHoveredImages(Array(properties.length).fill(false));
    }
  }, [properties]);
        ...
        {properties.map((property, index) => (
          <div className="flex flex-col lg:flex-row justify-between border border-slae-300 rounded-lg p-8 gap-5" key={property._id}>
            <div className="lg:w-1/3 relative" 
              ...
              <ImageCarousel imageUrls={property.imageUrls} isHovered={hoveredImages[index]} Delete={false} indexFound={0} onCarouselDataChange={dummyFunction}/>
            </div>
